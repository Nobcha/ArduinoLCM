// Arduino LC meter V2.3 Changed display format
// V2.2 2021.09.17, i2cLCD_PCF8574_0x27@2021.11.15,
// V2.1 Displaying with standard function  Gate time 200ms
// Franclin OSC timing Adopted dtostrf for xxx.xxxuH, decrease delay value
// 
// V2.0 2020.07.09, f1 ok, f2 ok, xxxxxxxpF xxxxxuH OK  
//
...
void setup() {
  ...
  Serial.println("LCM on i2cLCD v2.3"); // Version 2.3
...
// Write
  ...
  LCDPuts("LC METER v2.3   ") ;        // [00H]
  Serial.println( "Remove CL & SWON" );  
  delay(20);    
  LCDSetCursor(0,1) ;                 // 2nd line [40H]
  LCDPuts("Remove CL & SWON") ;        // [40H]
...
// Measurement starting check
  ...
  {    
    ...
    delay(100);
    LCDSetCursor(0,0);             //  Move cursur to the 8th of 1st line
    if(digitalRead(SEL_SW) == 0){  // If SW==0, start L measuring     
      LCDPuts("SET INDUCTER     ");   //  Ask setting L 
    }
    else LCDPuts("SET CAPACITOR   "); 
...
/*  Calculating value whether C position or L */
    ...
    {                                       // L position
      if ( freq3> 1000 ){
        ...
        Serial.println(f_rate);
        LCDPuts("L=");
        dtostrf( test_value, 8, 2, charbuf); // Changed to use dtostrf(**)
        LCDPuts(charbuf);
        LCDPuts("uH          ");     
    ...
    else
    {
      ...
      Serial.println("pF");
      if( test_value <= 0 ){
        LCDPuts(" No capacitor    ") ;        
      }
      else {
        LCDPuts("C=");
//      sprintf( charbuf, "%u", (long) (test_value ));
        dtostrf( test_value, 8, 1, charbuf); // Changed to use dtostrf(**)
        LCDPuts(charbuf);
        LCDPuts("pF           "); 
      }
    }
